...
<script>
  ...
  let targets = [];
  let attacks = [];
  let hasAttack = data.item.hasAttack;
...
<ApplicationShell bind:elementRoot>
  ...
  <section class="content">
    ...
    <p>{localize("ASE.MissileDialogMissileCountLabel")}
      <b>{missilesRemaining}</b> {data.effectOptions.missileType}{missilesRemaining !== 1 ? 's' : ''}
    </p>
    <table id="targetsTable" width="100%">
      ...
      {#each targets as target,i (target.token)}
        <tr on:mouseenter="{target.token._onHoverIn()}" on:mouseleave="{target.token._onHoverOut()}">
          <td>
            <label for="{target.token.document.id}-missiles"><img alt="Token" src="{target.token.document.texture.src}"
                                                                  width="30" height="30" style="border:0px">
              - {target.token.document.name}</label>
          </td>
          <td>
            <input style='width: 2em;' type="number" id="{target.token.document.id}-missiles" readonly
                   value="{target.missilesAssigned}">
          </td>
          <td>
